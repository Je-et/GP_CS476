...
function Cart() {
  ...
  useEffect(() => {
    checkLoginStatus();
    performance.mark('fetchCartItems-start');
    fetchCartItems().then(() => {
      performance.mark('fetchCartItems-end');
      performance.measure('fetchCartItems', 'fetchCartItems-start', 'fetchCartItems-end');

      const fetchCartItemsTime = performance.getEntriesByName('fetchCartItems')[0].duration;
      console.log(`Fetching cart items took: ${fetchCartItemsTime} ms`);
    })
  }, []);
